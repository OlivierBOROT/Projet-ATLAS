...
def geocode_commune(row, geolocator, retries=3):
    ...
    query = f"{row['nom_commune']}, {row['code_postal']}, {row['nom_departement']}, France"
...
def add_gps_to_communes(limit=None, region=None):
    ...
    print(f"\n⏱️  Estimation:")
    print(f"   • Temps: ~{estimated_minutes:.0f} minutes ({estimated_minutes/60:.1f} heures)")
    print(f"   • Limite Nominatim: 1 requête/seconde")
    
    # Afficher échantillon
    print(f"\n📋 Échantillon des communes à géocoder:")
    for i, row in df.head(5).iterrows():
        print(f"   {i+1}. {row['nom_commune']} ({row['code_postal']}) - {row['nom_departement']}")
            ...
            with engine.begin() as conn:
                conn.execute(
                    text("""
                        ...
                        WHERE commune_id = :id
                    """),
                    {"lat": lat, "lon": lon, "id": row['commune_id']}
                )
            success_count += 1
        else:
            fail_count += 1
...
def main():
    ...
    parser.add_argument(
        "--limit",
        type=int,
        help="Limiter à N communes (pour test)"
    )
